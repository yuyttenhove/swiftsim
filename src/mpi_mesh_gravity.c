 ...
 */
__attribute__((always_inline, const)) INLINE static size_t row_major_id_periodic_size_t(
    const size_t i, const size_t j, const size_t k, const size_t N) {
...
void accumulate_local_gparts_to_hashmap(const int N, const double fac, 
  ...
  for(int icell=0; icell < nr_local_cells; icell+=1) {
    ...
    for(int ipart=0; ipart < cell->grav.count; ipart+=1) {
      ...
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 0, j + 0, k + 0, N), mass * tx * ty * tz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 0, j + 0, k + 1, N), mass * tx * ty * dz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 0, j + 1, k + 0, N), mass * tx * dy * tz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 0, j + 1, k + 1, N), mass * tx * dy * dz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 1, j + 0, k + 0, N), mass * dx * ty * tz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 1, j + 0, k + 1, N), mass * dx * ty * dz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 1, j + 1, k + 0, N), mass * dx * dy * tz);
      add_to_hashmap(map, (hashmap_key_t) row_major_id_periodic_size_t(i + 1, j + 1, k + 1, N), mass * dx * dy * dz);
 ...
 *
 */
void hashmap_copy_elements_mapper(hashmap_key_t key, hashmap_value_t *value, void *data) {
...
void exchange_mesh_cells(size_t *nr_send, struct mesh_key_value *sendbuf,
  ...
  *nr_recv_tot = 0;
  ...
  if (swift_memalign("mesh_recvbuf", (void **)recvbuf, 32, *nr_recv_tot * sizeof(struct mesh_key_value)) != 0)
...
void hashmaps_to_slices(const int N, const int Nslice, hashmap_t *map, double *mesh) {
  ...
  if (swift_memalign("mesh_sendbuf", (void **)&mesh_sendbuf, 32, nr_send_tot * sizeof(struct mesh_key_value)) != 0)
  ...
  for(size_t i = 0; i < nr_send_tot; i+=1) {
    ...
    while((mesh_x >= slice_offset[dest_node] + slice_width[dest_node]) || (slice_width[dest_node] == 0)) {
      dest_node += 1;
    }
    nr_send[dest_node] += 1;
  }
