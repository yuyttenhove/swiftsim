...
void engine_marktasks_mapper(void *map_data, int num_elements,
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    if (t_type == task_type_self || t_type == task_type_sub_self) {
      ...
      else if (t_type == task_type_self && t_subtype == task_subtype_boundary) {
        if (ci_active_hydro) scheduler_activate(s, t);
      }

      else if (t_type == task_type_sub_self &&
               t_subtype == task_subtype_boundary) {
        if (ci_active_hydro) scheduler_activate(s, t);
      }
    ...
    /* Pair? */
      ...
      if ((t_subtype == task_subtype_density ||
           t_subtype == task_subtype_gradient ||
           t_subtype == task_subtype_boundary ||  /* boundary_loop */
          ...
          if (ci_active_hydro) {
            ...
            if (cj_active_hydro) {
              ...
              scheduler_activate_send(s, cj->mpi.send, task_subtype_gradient,
                                      ci_nodeID);
              scheduler_activate_send(s, cj->mpi.send, task_subtype_boundary, /* boundary_loop */
                                      ci_nodeID);
#endif
            }
          }
          ...
          if (cj_active_hydro) {
            ...
            if (ci_active_hydro) {
              ...
              scheduler_activate_send(s, ci->mpi.send, task_subtype_gradient,
                                      cj_nodeID);
              scheduler_activate_send(s, ci->mpi.send, task_subtype_boundary, /* boundary_loop */
                                      cj_nodeID);
#endif
            }
          }
    ...
    /* Hydro ghost tasks ? */ /* boundary_loop */
    else if (t_type == task_type_ghost || t_type == task_type_extra_ghost || t_type == task_type_boundary_ghost ||
