...
/* boundary_loop */
void pairs_all_boundary(struct runner *r, struct cell *ci, struct cell *cj) {
  ...
  for (int i = 0; i < ci->hydro.count; ++i) {
    ...
    for (int j = 0; j < cj->hydro.count; ++j) {
      ...
      if (r2 < hig2 && !part_is_inhibited(pj, e)) {
        ...
        /* Interact */
        runner_iact_nonsym_boundary(r2, dx, hi, hj, pi, pj, a, H);
      }
    }
  }
  ...
  for (int j = 0; j < cj->hydro.count; ++j) {
    ...
    for (int i = 0; i < ci->hydro.count; ++i) {
      ...
      if (r2 < hjg2 && !part_is_inhibited(pi, e)) {
        ...
        /* Interact */
        runner_iact_nonsym_boundary(r2, dx, hj, pi->h, pj, pi, a, H);
      }
    }
  }
}
...
/* boundary_loop */
void self_all_boundary(struct runner *r, struct cell *ci) {
  float r2, hi, hj, hig2, hjg2, dxi[3];  //, dxj[3];
  struct part *pi, *pj;
  const struct engine *e = r->e;
  const struct cosmology *cosmo = e->cosmology;
  ...
  for (int i = 0; i < ci->hydro.count; ++i) {
    ...
    for (int j = i + 1; j < ci->hydro.count; ++j) {
      ...
      if (r2 < hig2 && part_is_active(pi, e) && !part_is_inhibited(pj, e)) {
        ...
        /* Interact */
        runner_iact_nonsym_boundary(r2, dxi, hi, hj, pi, pj, a, H);
      }
      ...
      if (r2 < hjg2 && part_is_active(pj, e) && !part_is_inhibited(pi, e)) {
        ...
        /* Interact */
        runner_iact_nonsym_boundary(r2, dxi, hj, hi, pj, pi, a, H);
      }
    }
  }
}
